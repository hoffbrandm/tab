...
Hash routes keep refresh and Back in the same place (`#/home`, `#/weeklies`, `#/monthlies`, `#/planned`, `#/annual`, `#/pots`, `#/payslips`, `#/ani`, `#/giving`, `#/tabs`, `#/friend/<id>`, `#/more`). **The bar carries five: Home, Weeklies, Monthlies, Planned, More.** It listed all eleven, which wrapped to three rows, took a third of a phone screen, and made every page clear 240px of padding to get past it. The four rooms the month actually runs on stay in the bar; the rest are places you visit now and then, so More lists them with a line each saying what they are for, then sign-in, gist status, people, and payslip category names.

Every room opens with **one title, named the way its tab is**, one short line, and the detail folded behind **How this works**. Rooms used to lead with an eyebrow, a title that mostly repeated it, and a paragraph of documentation that is read once and then re-read on every visit for ever.
